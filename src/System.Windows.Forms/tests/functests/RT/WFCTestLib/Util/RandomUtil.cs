...
namespace WFCTestLib.Util
{
    ...
    public class RandomUtil
    {
        ...
        // <doc>
        // <desc>
        //  The largest value for the size paramter when creating a font.
        //  Note: this is not necessarily Points
        // </desc>
        // <seealso member="GetFont"/>
        // </doc>
        internal int MAX_POINT_SIZE = 96;
        ...
        // <doc>
        // <desc>
        //  The list of culture codes that the CultureInfo class supports for Win7/win2k8 server.
        // Due to culture code changes as part of Win7/win2k8 server and .NET 4.0 we now have to have a seperate array for Win7/win2k8 server
        // </desc>
        // <seealso member="GetIFormatProvider"/>
        // </doc>
        // created: v-ziyang:11/13/2012
        private string[] cultureFormatsWin7 = new string[]
        {
            "",
			"af",
			"af-ZA",
        ...
        public virtual DateTime GetDateTime()
        {
            ...
            DateTime retVal = GetDateTime(cal.MinSupportedDateTime, cal.MaxSupportedDateTime);
            LogValueIfLogging(retVal);
            return retVal;
        }                                                                                                   
        ...
        public virtual Image GetImage(ImageStyle type)
        {
            ...
            String fileName = Path.Combine(@"\\mdfile\whidbey\files\core\TeamData\Client\VSS\shadow\WinFormsRuntime\Libs\WFCTestLib\Util\Images", ImageNames[(int)type]);
...
//#else
        ...
        public virtual Icon GetIcon()
        {
            ...
            String fileName = Path.Combine(@"\\mdfile\whidbey\files\core\TeamData\Client\VSS\shadow\WinFormsRuntime\Libs\WFCTestLib\Util\Images", IconNames[index]);
        ...
        // </retvalue>
        // </doc>
        // [v-torend 8/31/2007 modify]
        // Fix for DDB# 126788. can handle the Surrogate characters in CHT.
        public virtual String GetString(int iMinLength, int iMaxLength, bool bValidOnly)
        {
            ...
            {
                //[v-torend]
                //Replace String.Substring() with this.SubString() 
                //retVal = strInterestingChars.Substring(0, length);
                retVal = this.SubString(strInterestingChars, 0, length);
                LogValueIfLogging(retVal);
                return retVal;
            }
            ...
            // insert interesting characters in various positions.
            strText = this.InsertRandom(strText, strInterestingChars);

            //[v-torend]
            //Replace String.Substring() with this.SubString()
            //retVal = strText.Substring(0, length);
            retVal = this.SubString(strText, 0, length);
            LogValueIfLogging(retVal);
            return retVal;
        }
        //public virtual String GetString(int iMinLength, int iMaxLength, bool bValidOnly)
        ...
        //[v-torend]
        /// <summary>
        /// Add 16-bit Unicode character or 32-bit Surrogate characters to ArrayList.
        /// Because Surrogate characters can not be destroyed while inserting or spliting.
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        private ArrayList TransformString2AL(String value)
        {
            value = this.SubString(value, 0, value.Length);
            ArrayList retAl = new ArrayList();            
            char[] chars = value.ToCharArray();
            for (int i = 0; i < chars.Length; i++)
            {
                if ((int)chars[i] >= 56320 && (int)chars[i] <= 57343)
                {
                    continue;
                }
                if ((int)chars[i] >= 55296 && (int)chars[i] <= 56319)
                {
                    retAl.Add(chars[i].ToString() + chars[i + 1].ToString());
                }
                else
                {
                    retAl.Add(chars[i].ToString());
                }
            }
            return retAl;
        }
        ...
        //[v-torend]
        /// <summary>
        /// Insert every character of Value to Target randomly.
        /// </summary>
        /// <param name="target"></param>
        /// <param name="value"></param>
        /// <returns></returns>
        private String InsertRandom(String target, String value)
        {
            //move this step to TransformString2AL() method
            //value = this.SubString(value, 0, value.Length);
            ArrayList alVal = this.TransformString2AL(value);
            ArrayList alTar = this.TransformString2AL(target);


            foreach (object o in alVal)
            {
                int range = this.GetRange(0, alTar.Count);
                range = range < alTar.Count ? range : alTar.Count - 1;
                alTar.Insert(range, Convert.ToString(o));
            }
            StringBuilder sbRet = new StringBuilder();
            foreach (object ret in alTar)
            {
                sbRet.Append(ret);
            }
        ...
        //[v-torend]
        /// <summary>
        /// Retrieves a substring of target. If we have separated the Surrogate characters at head or tail
        ///  of System.String, we should remove the single Surrogate character.
        /// </summary>
        /// <param name="target"></param>
        /// <param name="startIndex"></param>
        /// <param name="length"></param>
        /// <returns></returns>
        private String SubString(String target, int startIndex, int length)
        {
            target = target.Substring(startIndex, length);
            int firstChar = (int)target.ToCharArray()[0];
            int lastChar=(int)target.ToCharArray()[target.Length-1];

            if (firstChar >= 56320 && firstChar <= 57343)
            {
        ...
        private Region pathRegion(Size size)
        {
            ...
            */
            ...
            gp.AddString("A", new FontFamily(fnt.Name), (int)fnt.Style,
        ...
        public IComponent GetIComponent()
        {
            Type[] ic = new Type[] 
            {
                typeof(ColorDialog), typeof(ColumnHeader), typeof(ContextMenu),
                typeof(DataGridBoolColumn), typeof(ErrorProvider),
                typeof(FontDialog), typeof(MainMenu), typeof(NotifyIcon), typeof(OpenFileDialog), 
                typeof(PageSetupDialog), typeof(PrintDialog), typeof(PrintPreviewDialog), typeof(SaveFileDialog),
                typeof(TabControl), typeof(ThreadExceptionDialog), typeof(Timer), typeof(ToolBarButton), 
                typeof(ToolTip),
        ...
        {
            Type[] types = {
				typeof(Button), typeof(CheckBox), typeof(CheckedListBox), typeof(ComboBox), typeof(DataGrid), 
				...
				typeof(StatusBar), typeof(TabControl), typeof(ToolBar), typeof(TrackBar), typeof(TreeView), 
        ...
        public virtual CultureInfo GetCultureInfo(bool allowNeutral)
        {
            ...
            if (!allowNeutral)
            {
                while (culture.IsNeutralCulture)
                {
                    ...
                    else if (Utilities.IsWin7)
                    {
                        culture = new CultureInfo(GetArrayElement(cultureFormatsWin7).ToString());
                    }
                    else
                    {
                        culture = new CultureInfo(GetArrayElement(cultureFormats).ToString());
