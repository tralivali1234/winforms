...
namespace WFCTestLib.Util
{
    ...
    {
        public static bool TrySetFileDialogAutoUpgrade(System.Windows.Forms.FileDialog fd, bool value)
        {
            Type fileDialogType = fd.GetType();
            PropertyInfo autoUpgradeEnabledProperty = fileDialogType.GetProperty("AutoUpgradeEnabled", BindingFlags.Public | BindingFlags.Instance, null, typeof(bool), Type.EmptyTypes, null);
            if (null == autoUpgradeEnabledProperty)
            { return false; }
            autoUpgradeEnabledProperty.SetValue(fd, value, null);
            return (bool)autoUpgradeEnabledProperty.GetValue(fd, null);
        }
        ...
        public static bool IsVista
        {
            ...
            get
            {
                // only 6.0 is Vista OS while we have other newer OSs 
                return Environment.OSVersion.Version.Major == 6 && Environment.OSVersion.Version.Minor == 0;
            }
        }

        /// <summary>
        /// Returns true if the current operating system is Win7 or Win2k8 server.
        /// </summary>
        ...
        public static string GetSdkRoot(string version)
        {
            ...
            {
                sdkRoot = (string)rk.GetValue("sdkInstallRoot");
            }
            else if (clrRoot.IndexOf("1.1.4322") != -1)
            {
                sdkRoot = (string)rk.GetValue("sdkInstallRootv1.1");
            }
            else
            {
                sdkRoot = (string)rk.GetValue("sdkInstallRootv2.0");
            }

            if (null == sdkRoot)
            {
                //Fixby: v-qizhao, Date: 2007/08/13
                //Description: if IA64 the registry key is "Software\Microsoft\Microsoft SDKs\.NETFramework\v2.0"
                //sdkRoot = (string)Registry.LocalMachine.OpenSubKey(@"Software\Microsoft\Microsoft SDKs\Windows").GetValue("CurrentInstallFolder");
                try
                {
                    //Modified by: v-jianzh, Date: 2012/06/11, try to get V8.0A SDK 
                    //sdkRoot = (string)Registry.LocalMachine.OpenSubKey(@"Software\Microsoft\Microsoft SDKs\Windows").GetValue("CurrentInstallFolder");
                    sdkRoot = (string)Registry.LocalMachine.OpenSubKey(@"Software\Microsoft\Microsoft SDKs\Windows\V8.0A").GetValue("InstallationFolder");
                    //Modified 
                }
                catch (Exception)
                {
                    sdkRoot = "d:\\program files\\microsoft.net\\sdk\\v2.0 64bit";
                }
            }
            return sdkRoot;
        }
        ...
        public static string GetServerName(string token)
        {
            ...
            dictionary.Add("ASPNET", "winformssrvvm01.redmond.corp.microsoft.com");
            ...
            {
                Console.WriteLine("No server Configuration file found. Using hard-coded vaules");
