...
namespace ReflectTools.AutoPME
{
    public abstract class XControl : XComponent
    {
        ...
        protected AccessibleRole GetDefaultAccessibleRoleForType(Type t)
        {
            ...
            else if (t == typeof(DataGrid))
                ar = AccessibleRole.Table;
            else if (t == typeof(DataGridTextBox))
            ...
            else if (t.Name == "InheritDataGrid")
                ar = GetDefaultAccessibleRoleForType(typeof(DataGrid));
        ...
        {
            if ((c = GetControl(p)) == null) return ScenarioResult.Fail;
            if (c.ToString() == "System.Windows.Forms.Integration.ElementHost")
                return new ScenarioResult(c.ImeMode == ImeMode.Disable, "ImeMode: " + c.ImeMode + "  Expected result: " + ImeMode.Disable, p.log);
        ...
        protected virtual ScenarioResult get_Location(TParams p)
        {
            ...
            // Splitter, StatusBar, ToolBar respect docking even when it's not visible
            if (c.Dock != DockStyle.None && (c.Visible || c is Splitter || c is StatusBar || c is ToolBar))
            {
      ...
      */
        ...
        */
                        private bool IsSpecialCaseCallWndProc(Control c)
        {
            ...
            if (c is ToolBar) return true;

            if (c is DataGrid) return true;
...
*/
        ...
        protected virtual ScenarioResult get_ContainsFocus(TParams p)
        {
            ...
            // and Focused will return false (e.g. when DataGrid textbox has focus).
            ...
            // Grid textbox gets focus when you call Focus().  Escape puts focus on the grid itself.
            if (c is DataGrid)
        ...
        //                              RichTextBox, ProgressBar, ToolBar)
...
//			new SecurityPermission(PermissionState.Unrestricted).Assert();
        ...
        // updated by v-olzhan, add 3 methods: get_DeviceDpi(), LogicalToDeviceUnits(Int32 value), ScaleBitmapLogicalToDevice(ref Bitmap logicalBitmap) in .Net 4.7 for some HDPI improvement.
        protected virtual ScenarioResult get_DeviceDpi(TParams p)
        {
            if ((c = GetControl(p)) == null) return ScenarioResult.Fail;

            return new ScenarioResult(c.DeviceDpi == 96);
        }

        protected virtual ScenarioResult LogicalToDeviceUnits(TParams p, Int32 value)
        {
            if ((c = GetControl(p)) == null) return ScenarioResult.Fail;

            return new ScenarioResult(value.Equals(c.LogicalToDeviceUnits(value)));
        }

        protected virtual ScenarioResult ScaleBitmapLogicalToDevice(TParams p, ref Bitmap logicalBitmap)
        {
            ...
            if ((c = GetControl(p)) == null) return ScenarioResult.Fail;
            c.ScaleBitmapLogicalToDevice(ref mapB);

            return new ScenarioResult(mapB.Size.Equals(mapA.Size));
        }

        // updated by v-olzhan, add 1 method:LogicalToDeviceUnits(Size value) in .Net 4.8 for HDPI bug 616661 fix.
        protected virtual ScenarioResult LogicalToDeviceUnits(TParams p, Size value)
        {
            if ((c = GetControl(p)) == null) return ScenarioResult.Fail;
        ...
        protected virtual ScenarioResult set_ContextMenu(TParams p, ContextMenu value)
        {
            return get_ContextMenu(p);
        }

        protected virtual ScenarioResult get_ContextMenu(TParams p)
        {
            c = GetControl(p);

            ContextMenu cm = c.ContextMenu;
            ContextMenu cm2 = null;

            if (p.ru.GetBoolean())
            {
                cm2 = new ContextMenu();
                cm2.MenuItems.Add(new MenuItem(p.ru.GetString(25, true)));
            }

            c.ContextMenu = cm2;
            return new ScenarioResult(cm2 == c.ContextMenu);
        }
        ...
        protected bool PreservesHeight(Control c, TParams p)
        {
            ...
            // ToolBar will always keep its height if AutoSize is true.
            if ((c is ToolBar) && ((ToolBar)c).AutoSize == true)
            {
                p.log.WriteLine("AutoSize = true");
                return true;
            }
            ...
            // StatusBar, ToolBar respect docking even when it's not visible
            if (PreHandleMode && ((c is Splitter) || (c is StatusBar) || (c is ToolBar)) && (dock == DockStyle.Left || dock == DockStyle.Right || dock == DockStyle.Fill))
                return true;
        ...
        protected bool PreservesWidth(Control c, TParams p)
        {
            ...
            // ToolBar will only keep its width if it's docked left or right and AutoSize == true.
            if ((c is ToolBar) && (dock == DockStyle.Left || dock == DockStyle.Right) && ((ToolBar)c).AutoSize == true)
            {
                p.log.WriteLine("AutoSize = true");
                return true;
            }
            ...
            // StatusBar, ToolBar respect docking even when it's not visible
            if (PreHandleMode && ((c is Splitter) || (c is StatusBar) || (c is ToolBar) || (c is ToolStrip)) && (dock == DockStyle.Top || dock == DockStyle.Bottom || dock == DockStyle.Fill))
                return true;
...
#pragma warning restore 618 // 618 = Obsolete
        ...
        {
            if (c is ButtonBase || c is Label || c is MonthCalendar || c is PictureBox || c is ProgressBar || c is ScrollBar || c is Splitter || c is StatusBar || c is ToolBar || c is TrackBar)
        ...
        protected virtual ScenarioResult get_PreferredSize(TParams p)
        {
            ...
            if (!(c is DateTimePicker))
            {
                //The length of the text cannot be too long on JPN OS, or it will hang at caculating Preferred Size
                if (Utilities.IsOSJapanese)
                    c.Text = p.ru.GetString(System.Windows.Forms.Screen.PrimaryScreen.Bounds.Width, true);
                else
            ...
            //Modified by v-jianzh Date:6/6/2012
            //According to DevDiv 405757:Win8 X64: System.Runtime.InteropServices.ExternalException will get an exception: "get a A generic error occurred in GDI+" after some property setting changed.
            //Which has be resolved as By design/External
            //The exception is raised because the length of the string passed into GDI+ is too long. 
            //Specifically, the length passed in GdipMeasureString is 0n32767. After deducting ignorable characters from it, the length is 0n32612. This number is greater than OTL_MAX_CHAR_COUNT (0n32000) in Dwrite. So in Dwrite!SubstituteOtlGlyphs, it fails the length check. Consequently, GDI+ returns a generic error to the application.
            //This constant hasn¡¯t been changed since Win 7. Therefore, it isn¡¯t a regression. We consider it as a "by design".

            if (c.Text.Length > 32000)
                c.Text = c.Text.Substring(0, 31999);
            //Modified End
            ...
            if (((c is ButtonBase) && (c.Text == "")) || (c is Form) || (c is GroupBox) || (c is Panel) || (c is ListView) || (c is PictureBox) || (c is ProgressBar) || (c is PropertyGrid) || (c is Splitter) || (c is StatusBar) || (c is TabControl) || (c is ToolBar) || (c is TrackBar) || (c is TreeView) || (c is PrintPreviewControl) || (c is ScrollBar) || (c is DataGrid) || (c is PropertyGrid))
                    pass = (c.PreferredSize.Width >= initPrefSize.Width) && (c.PreferredSize.Height == initPrefSize.Height);
