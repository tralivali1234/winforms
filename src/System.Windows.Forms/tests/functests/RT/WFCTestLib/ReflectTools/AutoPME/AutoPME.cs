...
namespace ReflectTools.AutoPME
{
    ...
    public abstract class AutoPME : ReflectBase
    {
        ...
        protected override void InitTest(TParams p) {
            ...
            if (mdLogWrapperException != null) {
                p.log.WriteLine();

                if (mdLogWrapperException is System.IO.FileNotFoundException)
                    p.log.WriteLine("*****\"" + MDLogWrapper.LogFilename + "\" was not found.  Won't check for NoBase run flag.");
                else {
                    p.log.WriteLine("*****Unexpected exception was thrown when trying to read the run flags: " + mdLogWrapperException);
                }
        ...
        // We store any exceptions thrown by MDLogWrapper in SetOptions so we can report it after
        // the log has been initialized.
        Exception mdLogWrapperException;

		protected override void SetOptions()
		{
			base.SetOptions();
			if (!IgnoreNoBaseRunFlag)
			{
				// We run with NoBase on if the "AutoPME: NoBase" run flag is present in the MDLog.
				// This can be overridden in individual testcases by overriding SetOptions() or by
				// using the IgnoreNoBaseRunFlag setting/command-line flag.
                try {
				    MDLogWrapper mdLog = MDLogWrapper.GetInstance();

				    if (mdLog != null && mdLog.IsRunFlagPresent(NoBaseRunFlagName))
				    {
					    noBaseRunFlagDetected = true;
					    this.NoBase = true;
				    }
                }
                catch ( Exception e ) {
                    // Store this exception so we can log it when the log has been initialized.
                    mdLogWrapperException = e;
                }
			}
		}
