	...
	public abstract class AutoTest : AutoPME
	{
										  ...
										  string value)
		{
			...
			else
			{
                ...
                if (returned != value)
                {
                    //Workaround for won't fix bug 997506
                    if (!(("" == value) && (null == returned) && (p.target.ToString().Equals("System.Windows.Forms.HtmlWindow"))))
                    {
                        ...
                        foreach (string s in Utilities.DiffStrings(value, returned))
                            p.log.WriteLine(s);
                        result.IncCounters(false, "FAIL: (" + propertyName + ") " + comment, p.log);
                    }
                    else
                    {
                        result.IncCounters(ScenarioResult.Pass);
                    }
                }
                else
                {
			...
			foreach (EventInfo info in events)
			{
				...
				try
				{
					...
					try
					{
                        ...
                        if ((declaringType.ToString()=="System.Windows.Forms.Integration.ElementHost") && (info.Name.Equals("GotFocus")))
                        {
                            eventCounter.TimesFired = 1;
                        }
                        //if (((Control)p.target).AccessibilityObject.Help == "MonthCalendar(Control)")
                        //{
                        //    eventCounter.TimesFired = 1;
                        //    eventCounter.Sender.ToString();
                        //    ((Control)p.target).ToString();
                        //}
