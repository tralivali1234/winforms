...
#pragma warning restore 618
        ...
        protected ReflectBase(String[] args) : base()
        {
            ...
            //This will remove all listeners(nothing happens on assert or write)
            //Debug.Listeners.Clear();
            new ReflectBaseTraceListener(this);
        ...
        protected virtual void InitTest(TParams p) {
			...
			{ p.log.WriteLine(" Unable to identify permissions"); }
		}
		...
		[SecurityPermission(SecurityAction.Assert, Unrestricted=true)]
		private void LogEvidence(TParams p)
		{
			new FileIOPermission(PermissionState.Unrestricted).Assert();
			Assembly entry = Assembly.GetEntryAssembly();
			p.log.WriteLine("CodeBase:\t" + entry.CodeBase);
		
			//foreach (object o in entry.Evidence)
			//{
			//	if (o is Zone)
			//	{ p.log.WriteLine(" Zone:\t" + ((Zone)o).SecurityZone.ToString()); }
			//	else if(o is System.Security.Policy.Url)
			//	{ p.log.WriteLine(" Url:\t" + ((System.Security.Policy.Url)o).Value); }
			//	else if (o is System.Security.Policy.StrongName)
			//	{ p.log.WriteLine(" StrongName:\t" + ((StrongName)o).Name); }
			//	else if (o is System.Security.Policy.Hash)
			//	{ }//Nothing interesting to display for Hash
			//	else
			//	{ p.log.WriteLine(" Unknown:\t" + o.GetType().Name); }
			//}
			//PermissionSet ps = SecurityManager.ResolvePolicy(entry.Evidence);

			p.log.Write(" Permission:\t"); 
			
			//if (ps.IsUnrestricted())
			//{ p.log.WriteLine("Unrestricted"); }
			//else if (HasPermission(new SecurityPermission(PermissionState.Unrestricted), ps))
			//{ p.log.WriteLine("FullTrust"); }
			//else if (HasPermission(new UIPermission(PermissionState.Unrestricted), ps))
			//{ p.log.WriteLine("LocalIntranet"); }
			//else if (HasPermission(new UIPermission(UIPermissionWindow.SafeTopLevelWindows), ps))
			//{ p.log.WriteLine("Internet"); }
			//else if (HasPermission(new SecurityPermission(SecurityPermissionFlag.Execution), ps))
			//{ p.log.WriteLine("Execution"); }
		}
        ...
        {
            CodeAccessPermission[] permissions = null;
            BeginSecurityCheck(permissions , expectedMethod);
        }
