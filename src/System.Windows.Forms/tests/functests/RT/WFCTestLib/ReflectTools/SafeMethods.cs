namespace ReflectTools {
	...
	{ return Microsoft.Win32.Registry.CurrentUser; }
        ...
        {
            return Assembly.GetEntryAssembly().EscapedCodeBase;
        }
...
//		[WebBrowserPermission(SecurityAction.Assert, Unrestricted=true)]
        ...
        [SecurityPermission(SecurityAction.Assert, Unrestricted = true)]
        public static void RunAsUser(MethodInvoker adminCode, string domain, string userName, string password, LogonType logonType)
        {
            if (null == adminCode)
            { throw new ArgumentNullException("adminCode"); }
...
#pragma warning disable 0618
                //The other overload is only obsolete because it is not supported on Win9x
                //This code path will only execute on platforms which support SecureString.
                RunAsUser(adminCode, domain, userName, ss, logonType);
#pragma warning restore //0618
        ...
        [SecurityPermission(SecurityAction.Assert, Unrestricted = true)]
        [Obsolete("This method does not work on Win9x as SecureString is not supported, user the overload that takes a string")]
        [EditorBrowsable(EditorBrowsableState.Never)]
        public static void RunAsUser(MethodInvoker adminCode, string domain, string userName, SecureString password, LogonType logonType)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    bool status = LogonUserW(userName, domain, passwordPtr, logonType, LogonProvider.Default, out token);
                    if (!status)
                ...
                {
                    //RevertImpersonate(ref token, ref context);
                    throw;
                }
                finally
                { //RevertImpersonate(ref token,ref context); }
...
/*			Batch=4,
		...
		//private static bool RevertImpersonate(ref SafeUserToken token, ref WindowsImpersonationContext context)
		//{
        ...
        [PermissionSetAttribute(SecurityAction.Assert, Name = "FullTrust")]
//        public static void DoCallBackAppDomainData(AppDomain domain, CrossAppDomainDelegate
...
//        }
				...
				// build failed due to FileDialogPlace does not declarate
        //[PermissionSet(SecurityAction.Assert, Unrestricted = true)]
        //public static FileDialogPlace CreateFileDialogPlace(string path)
        //{
        //    return new FileDialogPlace(path);
        //}
        //[PermissionSet(SecurityAction.Assert, Unrestricted = true)]
        //public static FileDialogPlace CreateFileDialogPlace(Guid identifier)
        //{
        //    return new FileDialogPlace(identifier);
        //}

        [PermissionSet(SecurityAction.Assert, Unrestricted = true)]
        public static void SetSaveFileDialogOverwritePrompt(System.Windows.Forms.SaveFileDialog sfd, bool b)
        {
            sfd.OverwritePrompt = b;
        }

	}

    /// <summary>
    /// This class defines File Node that represents single file.
    /// </summary>
    public class FileNode : Object
    {
        ...
        public string Text;
        public FileNode() { }
        public FileNode(string st) { Name = st; }
        ...
        private void Stub()
        {
            if (File.Exists(Name))
                File.Delete(Name);
            FileStream fs = File.Create(Name, 1024);
    ...
    /// <summary>
    /// This class defines Folder Node that represents folder with content defined by
    /// field 'children'.
    /// </summary>
    public class FolderNode : FileNode
    {
        public FileNode[] children;
        public FolderNode(FileNode[] ch) { children = ch; }
        ...
        private void Stub()
        {
            string initP = Environment.CurrentDirectory;
            if (Directory.Exists(Name))
                Directory.Delete(Name, true);
            Directory.CreateDirectory(Name);
            Environment.CurrentDirectory = Path.Combine(Environment.CurrentDirectory, Name);
            foreach (FileNode fn in children)
                fn.Build();
            Environment.CurrentDirectory = initP;
        }
    }
